...
def parse_linear_gradient(node, transform, defs):
    ...
    return color.LinearGradient(begin, end, offsets, stop_colors)
...
def parse_radial_gradient(node, transform, defs):
    ...
    return color.RadialGradient(begin, end, offsets, stop_colors)
...
def parse_common_attrib(node, transform, fill_color, defs):
    ...
    if 'style' in attribs:
        ...
        if isinstance(fill_color, color.LinearGradient):
        ...
        if isinstance(stroke_color, color.LinearGradient):
...
def parse_shape(node, transform, fill_color, shapes, shape_groups, defs):
    ...
    if tag == 'path':
        ...
        paths = shape.from_svg_path(d, new_transform, force_closing)
        ...
        shape_groups.append(shape.ShapeGroup(\
            shape_ids = shape_ids,
        ...
        polygon = shape.Polygon(pts, force_closing)
        ...
        line = shape.Polygon(points, False)
        ...
        circle = shape.Circle(radius = torch.tensor(radius),
                                 center = center)
        ...
        rect = shape.Rect(p_min = p_min, p_max = p_max)
