...
def generate_single_affine_parameters(center, h, w, use_affine_norm=False, gt=None, device="cuda"):
    ...
    circle_center_norm = np.array([0.4935, 0.4953])  # 标准圆心的归一化坐标
    circle_center = circle_center_norm * h  # 转换为像素坐标

    if use_affine_norm:
        center_norm = center * 1.0 / h
        tx_val = center_norm[0] - circle_center_norm[0]  # 归一化x方向平移量
        ty_val = center_norm[1] - circle_center_norm[1]  # 归一化y方向平移量
    else:
        tx_val = float(center[0]) - circle_center[0]  # x方向平移量(像素)
        ty_val = float(center[1]) - circle_center[1]  # y方向平移量(像素)

    tx = torch.tensor(tx_val, dtype=torch.float32,
                      device=device, requires_grad=True)  # 可训练的x平移参数
    ty = torch.tensor(ty_val, dtype=torch.float32,
                      device=device, requires_grad=True)  # 可训练的y平移参数

    theta = torch.tensor(0.0, device=device, requires_grad=True)  # 可训练的旋转角度参数
    s = torch.tensor(0.11, device=device, requires_grad=True)  # 可训练的缩放比例参数
    ...
    if (gt is None):
        fill_color_init = torch.tensor(np.concatenate((npr.uniform(size=[3]), [
                                       np.random.uniform(0.7, 1)])), dtype=torch.float32, device=device, requires_grad=True)  # 可训练的随机RGBA颜色参数

    else:
        fill_color_init = list(gt[0, :, href, wref]) + [1.]
        fill_color_init = torch.FloatTensor(
            fill_color_init).to(device).requires_grad_(True)  # 可训练的基于GT的RGBA颜色参数
