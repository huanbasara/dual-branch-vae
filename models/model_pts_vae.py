...
class SVGTransformer(nn.Module):
    def __init__(self, cfg: _DefaultConfig):
        ...
        if cfg.use_resnet: # default: False
            self.resnet = ResNet(cfg.d_model)

        if cfg.use_vae: # default: False
            self.vae = VAE(cfg)
        else:
        ...
        if cfg.use_vqvae: # default: False
    ...
    def forward(self, args_enc, args_dec, ref_img=None, label=None, z=None, return_tgt=True, encode_mode=False,  return_indices=False):
        ...
        if z is None:
            ...
            if self.cfg.use_resnet: # default: False
                z = self.resnet(z)

            if self.cfg.use_vae: # default: False
            ...
            if self.cfg.use_vqvae: # default: False
